    ...
    public static class E3dToolDefinitions
    {
        ...
        static List<ToolDefinition> Build()
        {
            return new List<ToolDefinition>
            {
                ...
                // ★2026-07-31 可制造性 —— AVEVA 自带的预制检查引擎（见 E3dFabCheck.cs 头注）。
                T("e3d_fab_check", "★**这根管做不做得出来**（管道预制可制造性检查，走 AVEVA 自带的 PipeFabricationAddin）。给 PIPE 路径, 回: 能不能预制 · 划出几个预制件(spool, 车间/现场) · **逐条具体理由**。规则是真工程规则: 首/末管段太短 · 两弯之间太短 · 弯管与碰撞面干涉 · 预焊法兰不支持 · 外径/管长无效 · 松散件 · **轨道焊/机械焊/手工焊各自的最小长度** · 超 PIP/PIF/PIL · 支管连接与虾米弯余量 · 各弯半径不一致 …★这些是'画得出来但做不出来'那一类问题, 我们自己一条都答不出。★「判不了」单列, **既不算过也不算不过**。⚠ 声明成写: `LoadSpools` 会不会在库里留临时元素**尚未验证**——按最危险取, 真机验完再放宽。本工具**从不**调 Create/Delete/Rename。",
                   P("action:string:pipe(这根管**本身**合不合预制规范, 默认) | machine(**这个车间的机器**做不做得了: 弯管机/焊机接不接 + 最大可制造管长)。★两个是不同问题——同一根管 A 厂做得了 B 厂做不了。可选。"),
                   P("names:string:元素路径, 逗号分隔。action=pipe 时**只吃 PIPE**（预制按管线划分）。必填。"),
                   P("max:integer:每根最多列多少个 spool / 多少条消息, 默认40。★超了会**明说已截断**。可选。"),
                   R("names")),
                ...
                // ★★2026-07-31 本批最大的一条：命令窗口输出的**第三条通道**。
                //   我曾把「拿不到」写成结论，用户点破：**「照道理不会的，一定有一个输出。」**
                //   我证明的只是那两条 .NET 通道拿不到。看到两条路走不通，不等于只有两条路。
                T("e3d_alpha_log", "★★**把命令窗口的打印拿回来**（走 PDMS 原生的 `ALPHA LOG <file> OVER` … `ALPHA LOG END`）。这是 **`$Q` / `Q ATT` / `Q SPEC` / `LIST` / `Q MEM` 那一家的正确通道** —— 它们的结果全打在命令窗口里，而 `PdmsOutputEvents` 真机实测**只流报错**、`Command.Result` 对 Q 家族**是空的**（第十八跑双通道实测）。★特别是 **`$Q`**：官方手册反复写「真机 `$Q` 为准」，它是**权威裁决器** —— 拿得到之后，「这条命令形态对不对」以后可以自问自答，不必再攒一批进内网试。★回执把三种情形**分开报**：拿到真实打印 / 文件没生成（路径形态、不认 ALPHA LOG、权限）/ 文件生成了但是空的（通道通了、这条命令确实没打印）—— **这三件的下一步完全不同**。⚠ **未上真机**（命令形态来自 PDMS 官方 Syntax Library 逐字例子，不是猜的）。",
                   P("command:string:要跑的命令（多行用换行符分隔）。它打印什么都会被录下来。必填。"),
